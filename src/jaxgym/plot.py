    ...
    for idx, comp in enumerate(model):
        ...
        if hasattr(comp, 'focal_length'):
            try:
                ...
                ax.hlines(y=z_bfp, xmin=xmin, xmax=xmax, colors='blue', linestyle='dashed', linewidth=1)
                # Move BFP label further right to avoid overlap
                ax.text(xmax + x_offset * 2, z_bfp, 'Back Focal Plane', va='center', ha='left', color='blue', fontsize=8, alpha=0.7)
                z_ffp = float(comp.z) - float(comp.focal_length)
                # Only draw the BFP line from xmin to xmax
                ax.hlines(y=z_ffp, xmin=xmin, xmax=xmax, colors='blue', linestyle='dashed', linewidth=1)
                # Move BFP label further right to avoid overlap
                ax.text(xmax + x_offset * 2, z_ffp, 'Back Focal Plane', va='center', ha='left', color='blue', fontsize=8, alpha=0.7)
            except Exception:
                pass
